...
class RNNCaptchaRecognition:
    ...
    def __init__(self):
        ...
        self.w_d = tf.Variable(tf.truncated_normal([128, 64], stddev=0.1))
        self.b_d = tf.Variable(tf.zeros(64))

        self.w_out = tf.Variable(tf.truncated_normal([64, MAX_CAPTCHA * CHAR_SET_LEN], stddev=0.1))
    ...
    def forward(self):
        ...
        dense = tf.reshape(y, [-1, self.w_d.get_shape().as_list()[0]])
        dense = tf.nn.sigmoid(tf.add(tf.matmul(dense, self.w_d), self.b_d))
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1000000):
            ...
            out_test, _, loss, acc = sess.run([recnet.out, recnet.optimizer, recnet.loss, recnet.accuracy],
