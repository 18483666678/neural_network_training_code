...
def crack_captcha_cnn():
    ...
    # 3 conv layer
    w_c1 = tf.Variable(tf.random_normal([3, 3, 1, 32], stddev=0.1))
    ...
    w_c3 = tf.Variable(tf.truncated_normal([3, 3, 64, 128], stddev=0.1))
    ...
    w_d = tf.Variable(tf.truncated_normal([8 * 20 * 128, 1024], stddev=0.1))
    ...
    w_out = tf.Variable(tf.truncated_normal([1024, MAX_CAPTCHA * CHAR_SET_LEN], stddev=0.1))
    # b_out = tf.Variable(b_alpha * tf.random_normal([MAX_CAPTCHA * CHAR_SET_LEN]))
    b_out = tf.Variable(tf.zeros([MAX_CAPTCHA * CHAR_SET_LEN]))
